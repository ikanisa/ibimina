...
function parseSession(row) {
    ...
    const session = {
        ...
        lastInteractionAt: ensureDate(row.last_interaction_at ?? row.lastInteractionAt ?? row.updated_at ?? new Date()),
    };
...
export function createAgentSessionStore(options) {
    ...
    if (options.driver === "redis") {
        if (!options.client && !options.url) {
            throw new Error("Redis agent session store requires either a client instance or connection URL");
        }
