 ...
 */
export class MTNRwandaSmsAdapter {
    name = "MTN Rwanda SMS";
    countryISO3 = "RWA";
    /**
    ...
    canHandle(input) {
        ...
        return ((lower.includes("mtn") || lower.includes("momo")) &&
    ...
    extractReference(text) {
        ...
        const match = text.match(/reference[:\s]+([A-Z]{3}\.[A-Z0-9]{3}\.[A-Z0-9]{3,4}\.[A-Z0-9]{3,4}\.[0-9]{3})/i);
        if (match)
            return match[1];
        // Look for reference pattern anywhere in text (5 parts)
        const patternMatch5 = text.match(/\b([A-Z]{3}\.[A-Z0-9]{3}\.[A-Z0-9]{3,4}\.[A-Z0-9]{3,4}\.[0-9]{3})\b/i);
        if (patternMatch5)
            return patternMatch5[1];
    ...
    calculateConfidence(txn) {
        ...
        if (txn.txn_id && txn.txn_id.length > 8)
            score += 0.2;
