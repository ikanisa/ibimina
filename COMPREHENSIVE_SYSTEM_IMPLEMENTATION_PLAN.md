...
**Scope:** Complete system with 4 apps + shared packages + SMS payment integration
...
2. **Staff Admin Android** (`apps/staff-admin-android`) 🆕 TO CREATE
   ...
   - SMS Access: Read payment notification SMS → Parse with OpenAI → Auto-allocate to users
...
### Problem Statement
Rwanda's mobile money services (MTN MoMo, Airtel Money) send SMS notifications when payments are received. We need to:
