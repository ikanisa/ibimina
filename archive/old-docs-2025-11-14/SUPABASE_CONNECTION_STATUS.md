...
All three applications (Admin PWA, Client PWA, Website) are properly configured
to connect to Supabase, but **full database testing requires the 89 SQL
...
- **Server Client**: `createSupabaseServerClient()` - For server components/API
  routes
- **Service Role Client**: `createSupabaseServiceRoleClient()` - For admin
  operations
...
**IMPORTANT**: The Supabase database schema does not yet exist because the 89
SQL migrations have not been run.

### Why Migrations Must Be Run Locally

1. **Network Limitation**: Replit environment cannot directly connect to
   Supabase database (IPv6/connectivity issues)
...
3. **Rotate credentials** - User noted credentials were posted in chat and
   should be rotated before production
