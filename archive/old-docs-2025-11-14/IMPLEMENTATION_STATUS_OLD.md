...
The Android app (`apps/admin/android`) was failing to build with multiple
errors:
...
**Recommendation:** Start with Docker + Nginx for full control, migrate to
Vercel if needed.
...
**Recommendation:** Firebase App Distribution for staff apps, Play Store for
client apps.
...
3. **iOS Priority:** Is iOS client app needed in Phase 1, or can we start
   Android-only?
...
1. `apps/admin/android/ANDROID_BUILD_FIX.md` - Comprehensive Android build fix
   guide
...
Would you like me to proceed with implementing the Staff/Admin PWA or the SMS
reconciliation feature first?
