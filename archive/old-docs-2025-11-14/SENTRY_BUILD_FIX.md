...
The build process was failing with DNS/network errors when trying to connect to
`o1.ingest.sentry.io`. This occurred because the Sentry webpack plugin
(`@sentry/nextjs` with `withSentryConfig`) was attempting to upload source maps
during build time, even when:

1. No Sentry DSN was configured
2. Network access to Sentry servers was blocked by firewall rules
3. Running in environments where Sentry monitoring was not needed (e.g., CI,
   local development)
...
The `withSentryConfig` wrapper was being applied unconditionally in the Next.js
configuration files:
...
This caused the Sentry webpack plugin to always attempt to connect to Sentry
servers during the build process, regardless of whether Sentry was actually
configured or needed.

## Solution

Modified the Next.js configuration to conditionally apply the `withSentryConfig`
wrapper only when a Sentry DSN is present in the environment:
...
if (hasSentryDsn) {
  ...
  console.log(
    "[next.config] Sentry DSN not configured - skipping Sentry build integration"
  );
...
- Console log message:
  `[next.config] Sentry DSN not configured - skipping Sentry build integration`
- Runtime Sentry initialization (in `sentry.*.config.ts` files) will also skip
  initialization
...
- **Note**: CI workflows don't set SENTRY_DSN, so builds will skip Sentry
  integration

### Local Development

- **Before**: Could fail if Sentry wasn't configured or network was restricted
- **After**: Works without Sentry configuration, developers can opt-in by
  setting DSN
- **Recommendation**: Don't set Sentry DSN in local `.env` unless actively
...
- **Production workflows** (e.g., `deploy-cloudflare.yml`): Continue to set
  `SENTRY_DSN_ADMIN` and `NEXT_PUBLIC_SENTRY_DSN_ADMIN` secrets, so Sentry
...
1. **Syntax validation**: All three Next.js config files have correct TypeScript
   syntax
...
No action needed. Builds will work without Sentry configuration. To enable
Sentry locally:
...
No action needed. Current CI workflows don't set Sentry DSN, so builds will
succeed.

### For production

No action needed. Deployment workflows already set Sentry DSN secrets, so
...
1. **Reliability**: Builds succeed even when Sentry infrastructure is
   unavailable
...
4. **Developer Experience**: Simpler local development without Sentry
   configuration
...
1. Build without Sentry DSN:
   ...
   Expected: Build succeeds, logs "Sentry DSN not configured - skipping Sentry
   build integration"
