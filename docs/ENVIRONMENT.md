...
| Preview    | `https://<branch>--ibimina.netlify.app` | Branch DB (`preview-*`)  | Netlify preview env vars                          | Auto-generated per PR; seeded via `supabase db branch`. |
| Staging    | `https://ibimina-staging.netlify.app` | `ibimina-staging`        | Netlify staging env; Supabase secrets set via CLI | Manual promotion target; gated by protected approvals.  |
| Production | `https://console.ibimina.rw`       | `ibimina-prod`           | Netlify production env; Supabase dashboard        | Requires completion of `GO_LIVE_CHECKLIST.md`.          |
