...
- **Authentication**: Staff console logins use Supabase sessions. The `requireUserAndProfile`
  guard rejects requests lacking a verified
  session.【F:apps/admin/lib/auth.ts†L1-L200】【F:apps/admin/app/(auth)/login/page.tsx†L1-L160】
...
- **Encryption**: Sensitive fields (PII) are encrypted client-side
  with AES-256-GCM.
...
- Security events (failed logins, device revocations) are emitted with distinct
  event names (`auth.failure`, `device.revoked`) and appear in the drain and
