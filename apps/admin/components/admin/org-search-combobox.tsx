...
export function OrgSearchCombobox({ type, value, onChange, placeholder, disabled, className }: OrgSearchComboboxProps) {
  ...
  const endpoint = useMemo(() => `/api/orgs/search?type=${encodeURIComponent(type)}&q=${encodeURIComponent(query)}`, [type, query]);
  ...
  const label = type === "MFI" ? t("admin.invite.assignMfi", "Assign MFI") : t("admin.invite.assignDistrict", "Assign District");
  const ph = placeholder ?? (type === "MFI" ? t("admin.invite.searchMfi", "Search MFIs") : t("admin.invite.searchDistrict", "Search Districts"));
