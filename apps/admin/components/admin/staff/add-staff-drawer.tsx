...
export function AddStaffDrawerTrigger({ organizations }: AddStaffDrawerTriggerProps) {
  ...
  return (
    <>
      ...
      {isOpen && <AddStaffDrawer organizations={organizations} onClose={() => setIsOpen(false)} />}
    </>
  );
}
...
function AddStaffDrawer({ organizations, onClose }: AddStaffDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="relative w-full max-w-2xl rounded-2xl border border-white/10 bg-gradient-to-br from-ink/95 to-night/95 p-8 shadow-2xl">
        <button
          onClick={onClose}
          className="absolute right-4 top-4 rounded-lg p-2 text-neutral-2 hover:bg-white/10 hover:text-neutral-0"
        >
          ✕
        </button>

        <h2 className="mb-6 text-2xl font-bold text-neutral-0">
          <Trans i18nKey="admin.staff.addNewStaff" fallback="Add new staff" />
        </h2>
