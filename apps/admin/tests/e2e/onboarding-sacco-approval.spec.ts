...
test.describe("Onboarding → Add SACCO → Ask to Join → Staff Approve → Members Visible", () => {
  test("complete member onboarding and approval flow", async ({ page, request }) => {
    ...
    const firstResult = page.locator("li").filter({ hasText: /Imbere/i }).first();
    ...
    const groupCard = page.locator("[data-testid='group-card']").first()
      .or(page.locator("article").filter({ hasText: /ikimina|group/i }).first());
    ...
    const joinRequestItem = page.locator("[data-testid='join-request']").first()
      .or(page.getByText(/join request/i).locator("..").first());
    
    if (await joinRequestItem.isVisible()) {
      // Click approve button
      const approveButton = joinRequestItem.getByRole("button", { name: /approve|accept/i })
  ...
  test("staff can see pending join requests in approvals", async ({ page, request }) => {
    ...
    const approvalsPanel = page.locator("[data-testid='approvals-panel']")
