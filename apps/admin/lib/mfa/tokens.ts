...
const sign = (payload: string, secret: string) => encode(crypto.createHmac("sha256", secret).update(payload).digest());

export const createSignedToken = <T extends Record<string, unknown>>(payload: T, secret: string) => {
