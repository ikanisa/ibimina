...
export async function POST(request: Request) {
  const { email, role, sacco_id: saccoId, send_email: sendEmail = true } = (await request.json().catch(() => ({}))) as {
    email?: string;
  ...
  if (role !== "SYSTEM_ADMIN" && !saccoId) {
    return NextResponse.json({ error: "sacco_id is required for non-admin roles" }, { status: 400 });
  }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { error: updateError } = await (supabase as any).from("users").update({ role, sacco_id: role === "SYSTEM_ADMIN" ? null : saccoId ?? null }).eq("id", userId);
  if (updateError) {
  ...
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const res = await (supabase as any).schema("app").from("org_memberships").upsert({ user_id: userId, org_id: saccoId, role }, { onConflict: "user_id,org_id" });
    ...
    try {
      ...
      if (url) {
        await fetch(url, {
          ...
          body: JSON.stringify({ to: email, subject: "Welcome to the Staff Console", html: `<p>Welcome.</p><p>Temporary password: <b>${password}</b></p><p>Set a new password at first login.</p>` }),
        }).catch(() => void 0);
      } else {
