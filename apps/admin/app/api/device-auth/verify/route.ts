...
export async function POST(req: NextRequest) {
  try {
    ...
    const requiredFields = ["ver", "user_id", "device_id", "session_id", "origin", "nonce", "ts", "scope", "alg"];
    const missingFields = requiredFields.filter(field => !(field in signed_message));
    
    if (missingFields.length > 0) {
      await logAuditEvent(supabase, "CHALLENGE_FAILED", deviceKey.user_id, deviceKey.id, challenge.id, {
    ...
    if (timeDiff > 120 * 1000) { // 2 minutes
      await logAuditEvent(supabase, "CHALLENGE_FAILED", deviceKey.user_id, deviceKey.id, challenge.id, {
      ...
      return NextResponse.json(
        { error: "Signature timestamp invalid" },
        { status: 400 }
      );
    }
    ...
    if (integrity_token) {
      ...
      await supabase
        ...
        .update({
          ...
          integrity_status: integrityStatus.meets_device_integrity ? "MEETS_DEVICE_INTEGRITY" : "FAILED",
      ...
      if (!integrityStatus.meets_device_integrity) {
        await logAuditEvent(supabase, "INTEGRITY_CHECK_FAILED", deviceKey.user_id, deviceKey.id, challenge.id, {
        ...
        await logAuditEvent(supabase, "INTEGRITY_CHECK_PASSED", deviceKey.user_id, deviceKey.id, challenge.id, {
          integrity_status: integrityStatus,
        });
      }
    }
    ...
    await logAuditEvent(supabase, "CHALLENGE_VERIFIED", deviceKey.user_id, deviceKey.id, challenge.id, {
...
async function verifySignature(
  ...
  try {
    ...
    if (algorithm === "ES256") {
      ...
      // Ed25519
      verify = crypto.createVerify(null); // Ed25519 doesn't use hash
    } else {
