...
1. **Client Applications**
   - `apps/admin`: Staff console (Next.js 15, App Router, React Server
     Components) deployed to Vercel.
   - `apps/member`: Member-facing PWA with offline cache and Workbox service
     worker, also deployed to Vercel.
   - `apps/mobile`: Expo-managed React Native app distributed via EAS channels.
2. **Backend Services**
   - Supabase Postgres with `app`, `identity`, and `operations` schemas protected
     by RLS. Materialized views surface dashboard aggregates.
...
3. **Integration Layer**
   - Webhooks from MTN/Airtel process through Edge Functions with signed payloads.
   - `packages/providers` exposes clients for mobile money, SMS, and notifications.
4. **Observability & Tooling**
   - GitHub Actions pipeline runs lint/tests/RLS proofs.
   - Prometheus + Grafana stack (see `infra/metrics/`) captures metrics; log drain
     exports from Vercel and Supabase feed dashboards.
...
- **Authentication**: Clients use Supabase Auth; server components hydrate session
...
- **Reporting**: Materialised views (`app.member_aggregates`, `app.payment_rollup`)
  served through RPC endpoints consumed by admin dashboards.
...
- **Runbooks**: Deployment, incident response, and Supabase operations covered in
  `GO_LIVE_CHECKLIST.md`, `docs/runbooks/SECURITY.md`, and `docs/operations-runbook.md`.
